...
void draw_rect_tsf(tile& dest, dbuf& ds, double dep,
	vector2 ct, double w, double h, mat2 tsf, drect vp, dcol col) {
	mat2 inv_tsf = tsf.inv();
	vector2 hwh_tsf = tsf.abs() * vector2(w, h) / 2;
	// 这个 1 是经验公式。我没做严格论证。
	...
	rep(i, a, b) rep(j, c, d) {
		...
		vector2 u_pnt = vector2(0.5) +
			inv_tsf * (vector2(d_pnt) - ct) / vector2(w, h);
		if (insd(u_pnt, { 1, 1 })) {
...
void draw_eclipse(tile& dest, dbuf& ds, double dep,
	vector2 ct, double ax, double ay, drect vp, dcol col) {
